...
-WARNING: The OS Optimization subscript is only available for Linux users at this time. (Tested on Xubuntu and Debian.) Furthermore, it must be noted that it must only be run ONE TIME due to the system file changes being made. Do not interrupt this section of the installer if selected! I repeat: using option '-t' more than one time will corrupt your system files! If this should occur, then simply rename the backups found in their same folders back to their originals by removing the file suffix '.bak' therefrom.
