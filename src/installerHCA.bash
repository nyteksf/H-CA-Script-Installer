...
sleep 1
  ...
  ## CUE EYE CANDY FOR THE HELL OF IT:
  declare -a Spinner
  Spinner=(/ - \\ \| / - \\ \| )
  Spinnerpos=0
  ...
  ## PREPARING TO SERVE UP THE 'MEAT AND POTATOES' PORTION OF OUR SCRIPT:
  sleep 3
  echo -n "Shell script..."
  sleep 1
  echo "ready and initialized!"
  sleep 1
  ...
  echo "It is true that Git sits at the very heart of Github, after all; a place you will soon become familiar with."
    ...
    echo -en "\E[31m"
  read -r -p "/   /   /  /// Enter in your first name: "
  git config --global user.name "$REPLY"

  echo -en "\E[31m"
  read -r -p "/   /   /  /// Carefully enter in your email address: "
  ...
  read -p "/   /   /  /// Were you able to enter in the two previous two commands successfully? [Y/n] " -n 1 -r
        ...
        then
            ...
            echo
            echo
  read -r -p "/   /   /  /// Did you find any *.pub files in directory '~/.ssh'? Enter [Y/n] to continue." -n 1
                  if [[ ! $REPLY =~ ^[Yy]$ ]]
                  then
                  read -r -p "/   /   /  /// We're making you a new SSH key. Please prepare to carefully type your Email Address."
           ...
           ## ADD YOUR SSH KEY TO GITHUB ACCT.
            ...
            echo
            read -r -p "/   /   /  /// Is your GitHub Account all set up and logged in? Press [Y/n] to continue." -n 1
            echo

            sudo apt-get install xclip
# Downloads and installs xclip. If you don't have `apt-get`, you might need to use another installer (eg, like 'yum')
...
# Copies the contents of the id_rsa.pub file to your clipboard. You'll need to paste it to the website soon after.
            ...
            echo >> ~/Desktop/README_GITHUB.txt
            echo "XClip has automagically coping your SSH Key to the clipboard so that it can be pasted directly into your GitHub account." >> ~/Desktop/README_GITHUB.txt
  ...
  ## The GIMP Image Editing Suite
  sudo apt-get install gimp

  ## LOAD INDICATOR IF DESIRED
  ...
  ## IN UBUNTU: ADDING USERNAME TO PANEL FOR ID
  gsettings set com.canonical.indicator.session show-real-name-on-panel true

  ## USE UNITY? 'UNITY-TWEAK-TOOL.'
  ...
  ## INSTALL MORE COMPRESSION AND DECOMPRESSION TOOLS
  sudo apt-get install -y sharutils uudeview mpack arj cabextract
  ...
  echo "Closing Notes:"
  echo
  echo "-You may now enter into the terminal: 'gksudo thunar /usr/share/xfce4/backdrops' to discover new and exciting desktop backgrounds."
...
read -r -p  "/   /  /  /// Press [Enter] only when ready, and XCode has been successfully installed." -n 1
...
## CONFIGURING GIT
  ...
  echo "It is true that Git sits at the very heart of Github, after all; a place you will soon become familiar with."
    ...
    echo -en "\E[31m"
  read -r -p "/   /   /  /// Please enter your first name: "
  git config --global user.name "$REPLY"

  echo -en "\E[31m"
  read -r -p "/   /   /  /// Carefully enter your email address: "
  git config --global user.email "$REPLY"
  echo
  sleep 2


  read -r -p "/   /   /  /// Were you able to enter in the two previous two commands successfully? [Y/n] " -n 1 -r
        if [[ $REPLY =~ ^[Yy]$ ]]
        then
            ## RUN THE COMMANDS
            clear
...
EOF
    ...
    read -r -p "/   /   /  /// |Press [Enter] when ready to continue installing..."
    # TO DO STILL:
    # CUSTOM H:CA GRUB BOOTSCREEN BACKGROUND:
    #     see: https://sites.google.com/site/easylinuxtipsproject/beautifygrub
    #
    ...
    (
     ...
     echo "#!/bin/sh" | sudo tee -a ~/tmp.xxxxx/wireless
     echo | sudo tee -a ~/tmp.xxxxx/wireless
     echo "/sbin/iwconfig wlan0 power off" | sudo tee -a ~/tmp.xxxxx/wireless
    ...
    echo "Ask a teacher if you don't know based on the output below, but the correct partition should be a type 'ext4', if that helps."
    ##
    echo -en "\E[37m"
    echo
    mount | grep sda
    ...
    ## ADDING SURICATA (aka "THE SNORT REPLACER")
    ...
    sudo touch ~/.bash_aliases ## Just in case; we come prepared.
    echo "alias update='sudo apt-get update && apt-get upgrade && apt-get autoclean && apt-get autoremove'" | sudo tee -a ~/.bash_aliases
...
echo "30 7 * * * /usr/sbin/tripwire --check | mail -s 'Weekly Tripwire report' inf@localhost" >> ~/Desktop/OSTWEAKS.txt
    ...
    echo 'alias fluxon="~/.flux/xflux -z 94124"' >> ~/.bash_aliases ##School zipcode is this
...
echo
    ...
    read -r -p "/   /  /  ///  Do you use the printer often? I ask, as we will be temporarily disabling listening service CUPS--a listening print service--for you, if not. Press [Y/n]."
        if [[ $REPLY =~ ^[Yy]$ ]]
        then
          ...
          ## INSTALL SNORT PREREQ. DEPENDENCIES
          sudo apt-get install -y rails
          sudo apt-get install -y default-jre
          sudo apt-get install -y default-jdk
          sudo apt-get install libxslt-dev libxml2-dev
          ## INSTALL SNORBY: SNORT GUI (IS USED WITH SURICATA TOO)
          sudo git clone git://github.com/Snorby/snorby.git ~/tmp.xxxxx
          cd ~/tmp.xxxxx || exit
          eval sudo bundle install
          ## UPDATING USER README
       ...
       ## INSTALLING BARNYARD2 FOR USAGE WITH SURICATA
       ...
       sudo make install
        ...
        (
         ...
         ## old line: cd /usr/lib/; sudo ln -s /usr/local/lib/libdnet.1.0.1 libdnet.1
        ...
        (
         ...
         cd ~/tmp.xxxxx/libcaca-0.99.beta19 || exit
         eval sudo ./configure
    ...
    read -r -p "/   /   /  /// |Press [Enter] key to reboot now..."
    clear
    sleep 1
    ...
    echo
    echo
    echo -r -n "/// / /  /  Going down for a system reboot..."
    sleep 2
